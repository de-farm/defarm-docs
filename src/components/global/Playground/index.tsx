...
const ControlButton = ({
  ...
  const controlButton = (
    <button
      ...
      disabled={disabled}
      className={`playground__control-button ${
        isSelected ? "playground__control-button--selected" : ""
      }`}
      onClick={handleClick}
      data-text={label}
    >
...
export default function Playground({
  ...
  if (typeof version === "undefined") {
    console.warn(
      'You must specify a `version` for the Playground example. For example: <Playground version="7" />'
    );
    return;
  }
  ...
  const hasUsageTargetOptions =
    (code[usageTarget] as UsageTargetOptions)?.files !== undefined;
  /**
  ...
  function openEditor(event) {
    ...
    if (!hasUsageTargetOptions) {
      ...
      editorOptions.dependencies = (
        code[usageTarget] as UsageTargetOptions
      ).dependencies;
    }
      ...
      return (
        <PlaygroundTabs groupId={usageTarget} className="playground__tabs">
          {Object.keys(codeSnippets[usageTarget]).map((fileName) => (
            ...
            >
              <div id={getCodeSnippetId(usageTarget, fileName)}>
