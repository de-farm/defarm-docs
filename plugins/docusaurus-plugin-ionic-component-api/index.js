...
module.exports = function (context, options) {
  return {
    ...
    async loadContent() {
      ...
      const preset = context.siteConfig.presets.find(
        (preset) => Array.isArray(preset) && preset[0] === "classic"
      );

      const docsPluginOptions = preset[1].docs;
      ...
      const generateMarkdownForVersion = async (
        ...
        const response = await fetch(
          `https://unpkg.com/@ionic/docs@${npmTag}/core.json`
        );
...
| --- | --- |
${events
  .map((event) => `| \`${event.event}\` | ${formatMultiline(event.docs)} |`)
  .join("\n")}
